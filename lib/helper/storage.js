...
class Storage {
  ...
  load(reload = false) {
    ...
    if (!self.loaded || reload) {
      let configText = atom.config.get('remote-editor.config');
      if (configText) {
      ...
      self.loaded = true;
      self.tree = null;
      self.version = atom.packages.getActivePackage('remote-editor').metadata.version;
      self.settings = {};
    }
  ...
  save() {
    ...
    atom.config.set('remote-editor.config', encrypt(self.password, JSON.stringify({
  ...
  loadServers(servers) {
    ...
    let sortServerProfilesByName = atom.config.get('remote-editor.tree.sortServerProfilesByName');
    servers.sort((a, b) => {
  ...
  migrate(configArray) {
    ...
    if (typeof configArray.version === 'undefined') {
      ...
      self.backup(atom.packages.getActivePackage('remote-editor').metadata.version);
  ...
  backup(version) {
    ...
    const dest = Path.dirname(atom.config.getUserConfigPath()) + Path.sep + 'remote-editor-config.cson.' + version;
