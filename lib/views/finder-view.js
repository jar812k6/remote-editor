...
export default class FinderView extends EventEmitter {
  ...
  constructor() {
    ...
    self.selectListView = new SelectListView({
      ...
      filterKeyForItem: (item) => {
        if (atom.config.get('remote-editor.finder.filterKeyForItem') == "Filename") {
          return item.file;
        } else {
      ...
      atom.config.observe('remote-finder.useAlternateScoring', (newValue) => {
        ...
        if (this.useAlternateScoring) {
          this.selectListView.update({
            filter: (items, query) => {
              return query ? fuzzaldrinPlus.filter(items, query, { key: atom.config.get('remote-editor.finder.filterKeyForItem') }) : items
            }
          })
        } else {
  ...
  show() {
    ...
    self.emit('remote-editor-finder:show');
  }
  ...
  hide() {
    ...
    self.emit('remote-editor-finder:hide');
  }
  ...
  open(item) {
    ...
    self.emit('remote-editor-finder:open', item);
  }
}
