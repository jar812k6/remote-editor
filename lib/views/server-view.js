...
class ServerView extends View {
  ...
  getId() {
    ...
    return 'remote-editor-' + md5(JSON.stringify(object));
  }
  ...
  expand() {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      self.getConnector().listDirectory(self.getPath()).then((list) => {
        ...
        if (!atom.config.get('remote-editor.tree.sortFoldersBeforeFiles')) {
          entries.sort((a, b) => {
            if (a.name.toLowerCase() < b.name.toLowerCase()) return -1;
            if (a.name.toLowerCase() > b.name.toLowerCase()) return 1;
            return 0;
  ...
  select(deselectAllOther = true) {
    ...
    if (deselectAllOther) {
      elementsToDeselect = $('.remote-editor-view .selected');
  ...
  refresh(elementToRefresh) {
    ...
    let sortFoldersBeforeFiles = atom.config.get('remote-editor.tree.sortFoldersBeforeFiles');
