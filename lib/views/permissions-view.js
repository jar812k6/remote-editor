...
import { rightsToPermissions, permissionsToRights } from './../helper/helper.js';

export default class PermissionsView extends View {

  static content() {
    return this.div({
      class: 'remote-editor permissions-view overlay from-top',
    }, () => {
      this.div({
        class: 'panels',
      }, () => {
  ...
  createPanelContent() {
    ...
    self.numericInput.getModel().buffer.onDidChange((obj) => {
      ...
      obj.changes.forEach((change) => {
        change.newText.split('').forEach((value) => {
  ...
  validate() {
    ...
    permissions.split('').forEach((value) => {
      if (allowed.indexOf(value) == -1) {
        isvalid = false;
      }
    });
